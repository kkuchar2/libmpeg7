...
void ContourShape::readFromXML(XMLElement * descriptorElement) {
    XMLElement * p_GlobalCurvature    = NULL;
    XMLElement * p_PrototypeCurvature = NULL;
    XMLElement * p_HighestPeakY       = NULL;

    std::vector<XMLElement *> p_Peaks;

    /* Iterate over children */
    for (XMLElement * child = descriptorElement->FirstChildElement(); child != NULL; child = child->NextSiblingElement()) {
    ...
    if (p_GlobalCurvature == NULL) {
        throw CONT_SHAPE_XML_GLOBAL_CURVATURE_MISSING;
    }

    if (p_HighestPeakY == NULL) {
...
std::string ContourShape::generateXML() {
    ...
    for (int i = 0; i < 2; i++) {
        globalCurvature_string.append(std::to_string(descriptorGlobalCurvatureVector[i]) + " ");
    }
    ...
    if (descriptorPeaksCount > 0) {
        ...
        for (int i = 0; i < 2; i++) {
            prototypeCurvature_string.append(std::to_string(descriptorPrototypeCurvatureVector[i]) + " ");
        }
    ...
    if (descriptorPeaksCount > 1) {
        std::vector<XMLElement *> peakNodes(descriptorPeaksCount - 1, NULL);
