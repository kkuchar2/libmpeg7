...
Descriptor * DominantColorExtractor::extract(Image & image, const char ** params) {
    ...
    unsigned char * alphaChannelBuffer = transparencyPresent ? image.getChannel_A() : NULL;
...
void DominantColorExtractor::rgb2xyz(Image &image, double * XYZ) {
    ...
    for (int currentRow = 0; currentRow < rows; currentRow++) {
        uchar * row_ptr = mat.ptr<uchar>(currentRow);

        for (int currentColumn = 0; currentColumn < cols; currentColumn += 3) {
            /* Modification of:
            r = (RGB[i]     <= 20) ? (double)(8.715e-4 * RGB[i])     : (double)pow((RGB[i]     + 25.245) / 280.245, 2.22);
            g = (RGB[i + 1] <= 20) ? (double)(8.715e-4 * RGB[i + 1]) : (double)pow((RGB[i + 1] + 25.245) / 280.245, 2.22);
            b = (RGB[i + 2] <= 20) ? (double)(8.715e-4 * RGB[i + 2]) : (double)pow((RGB[i + 2] + 25.245) / 280.245, 2.22); 
            
            It was Rec. 709 algorithm for RGB to sRGB conversion.


            Now, same recommendation is used in lookup table - 200 ms faster and direcly is using mat object*/

            b = rgb_pow_table[row_ptr[currentColumn]];
            g = rgb_pow_table[row_ptr[currentColumn + 1]];
            r = rgb_pow_table[row_ptr[currentColumn + 2]];

            XYZ[i]     = 0.412453 * r + 0.357580 * g + 0.180423 * b;
            XYZ[i + 1] = 0.212671 * r + 0.715160 * g + 0.072169 * b;
            XYZ[i + 2] = 0.019334 * r + 0.119193 * g + 0.950227 * b;
