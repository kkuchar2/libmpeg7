...
(JNIEnv * env, jobject, jint descriptorType, jstring imgURL, jobjectArray parameters) {
    ...
    const char * imgPath = NULL; // Path to image
    const char ** params = NULL; // Parameters
    ...
    // Parse image path
    imgPath = env->GetStringUTFChars(imgURL, (jboolean *) false);
...
(JNIEnv * env, jobject, jstring xml1, jstring xml2, jobjectArray parameters) {
    ...
    const char *  xml1_str = NULL; // xml1
    const char *  xml2_str = NULL; // xml2
    const char ** params   = NULL; // parameters
    ...
    // Parse xmls
    xml1_str = env->GetStringUTFChars(xml1, (jboolean *) false);
    xml2_str = env->GetStringUTFChars(xml2, (jboolean *) false);

    if (xml1_str == NULL || xml2_str == NULL) {
        return createJniMessage(env, message(JNI_XML_NULL));
    }
...
const char ** getParameters(JNIEnv * env, jobjectArray parameters) {
    ...
    // 2, 3 - Parameters size = 1
        ...
        // 3. Single value != NULL
            ...
            params = new const char *[stringCount + 1];
            params[0] = env->GetStringUTFChars(string, (jboolean *) false);
            params[1] = NULL;
        }
    }
    // 4. Paramters size > 1
        ...
        for (int i = 0; i < stringCount; i++) {
            ...
            // Otherwise get value
            params[i] = env->GetStringUTFChars(string, (jboolean *) false);
        }
        params[stringCount] = NULL;
    }
...
unsigned char * getImageBuffer(JNIEnv * env, jbyteArray data, int size) {
    ...
    jbyte * jniBufferPtr = env->GetByteArrayElements(data, (jboolean *) false);
