...
void TextureBrowsing::readFromXML(XMLElement * descriptorElement) {
    XMLElement * p_Regularity = NULL;
    XMLElement * p_Direction1 = NULL;
    XMLElement * p_Scale1     = NULL;
    XMLElement * p_Direction2 = NULL;
    XMLElement * p_Scale2     = NULL;
    ...
    for (XMLElement * child = descriptorElement->FirstChildElement(); child != NULL; child = child->NextSiblingElement()) {
        ...
        else if (!name.compare("Direction") && p_Direction1 == NULL) {
            p_Direction1 = child;
        }
        else if (!name.compare("Direction") && p_Direction1 != NULL) {
            p_Direction2 = child;
        }
        else if (!name.compare("Scale") && p_Scale1 == NULL) {
            p_Scale1 = child;
        }
        else if (!name.compare("Scale") && p_Scale1 != NULL) {
            p_Scale2 = child;
    ...
    if (p_Regularity == NULL) {
        throw TEXT_BROWS_XML_REGULARITY_MISSING;
    }

    if (p_Direction1 == NULL) {
